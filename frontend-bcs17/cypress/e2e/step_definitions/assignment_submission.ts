...
Given('Student Alex is enrolled in the {string} course', (course: string) => {
  cy.log(`Student Alex is enrolled in ${course} course`);
});

Given('there is an assignment titled {string} due on {string}', (title: string, dueDate: string) => {
...
Given('Student Alex navigates to the submission page for {string}', (assignmentTitle: string) => {
  cy.visit('/assignments');
...
// Action steps
When('Student Alex enters {string} in the submission comments', (comments: string) => {
  cy.get('[data-testid="submission-comments"]').type(comments);
});

When('Student Alex uploads file {string} as submission', (fileName: string) => {
...
When('Student Alex clicks the {string} button', (buttonText: string) => {
  cy.contains('button', buttonText).click();
});

When('the due date has passed for assignment {string}', (assignmentTitle: string) => {
...
// Submission steps
When('Student Alex enters {string} as the submission content', (content: string) => {
  cy.get('[data-testid="submission-content"]').clear({ force: true }).type(content, { force: true });
});

When('Student Alex uploads {string} as an attachment', (fileName: string) => {
...
Then('the submission should include {int} file(s)', (fileCount: number) => {
...
Given('Student Alex has a saved draft for assignment {string}', (assignmentTitle: string) => {
  cy.visit('/assignments');
  cy.contains(assignmentTitle).click();
...
Given('Student Alex has already submitted assignment {string}', (assignmentTitle: string) => {
  cy.visit('/assignments');
  cy.contains(assignmentTitle).click();
...
When('Student Alex tries to upload file {string} with invalid format', (fileName: string) => {
  cy.get('[data-testid="submission-file-upload"]').attachFile(fileName);
});
